...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Pacemaker_OperatingModes_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    Pacemaker_OperatingModes_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(Pacemaker_OperatingModes_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Pacemaker_OperatingModes_M, "stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  /* Terminate model */
  Pacemaker_OperatingModes_terminate();
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(Pacemaker_OperatingModes_M, 0);

  /* Initialize model */
  Pacemaker_OperatingModes_initialize();
